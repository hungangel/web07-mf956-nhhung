...
namespace MISA.AMIS.Core.Services.CommonFunctions
{
    public static class CommonFn
    {
        ...
        public static string GetDislayStyle(Type objectDataType)
        {
            string numberType = "integer decimal float";
            if (numberType.Contains(objectDataType.Name.ToLower()))
                return "AlignRight";
            if (objectDataType.FullName.Contains("DateTime"))
                return "AlignCenter";
            return "AlignLeft";
        }
    }
